...
import { ArrowLeft, Edit, Trash2, Copy, DollarSign, CalendarDays, FileText, LinkIcon as LinkIconLucide, MoreHorizontal, ChevronLeft, ChevronRight, RotateCcw, Play, Pause, Loader2 } from 'lucide-react';
...
const PaymentLinkDetailsPage: NextPage = () => {
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      // paymentLink state will update via onSnapshot listener
    } catch (error) {
  ...
  const InfoItem = ({ icon: Icon, label, value, isLink }: { icon: React.ElementType, label: string, value?: string | React.ReactNode, isLink?: boolean }) => (
    <div className="flex items-start space-x-3"> <Icon className="h-5 w-5 text-muted-foreground mt-0.5" />
      <div> <p className="text-sm text-muted-foreground">{label}</p>
        {isLink && typeof value === 'string' ? ( <Link href={value} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-primary hover:underline break-all">{value}</Link> ) : 
              ...
              <div className="mt-4 sm:mt-0 flex space-x-2">
                 <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={handleCopyLink}><Copy className="mr-2 h-4 w-4" /> Copy Shareable Link</DropdownMenuItem>
                ...
                <TableBody>
                  {paginatedTransactions.map((txn) => (
                    <TableRow key={txn.id} className="h-[60px]">
                      <TableCell className="px-4 py-2 text-sm text-muted-foreground">{formatDate(txn.date)}</TableCell>
